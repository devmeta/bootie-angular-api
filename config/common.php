...
function timespan($ts,$mysterious=true) 
{
	$mins = (time() - $ts) / 60;
	$mins = round($mins);
	$span="";
	...
	{ // meses
		$ratio = round($mins / 40320);
			if($ratio%12==0 OR $ratio==12)
				$span = "casi " . floor($ratio/12) . " año".($ratio>12?'s':'');
			else
				if($mysterious)
					$span = $ratio . " mes".($ratio>1?'es':'');
				else
					$span = date('Y M d',$ts);
